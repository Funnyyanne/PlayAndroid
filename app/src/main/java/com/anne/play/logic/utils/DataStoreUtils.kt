...
val Context.datsStore: DataStore<Preferences> by preferencesDataStore(name = "PlayAndroidDataStore")

object DataStoreUtils {
    private lateinit var dataStore: DataStore<Preferences>
    fun init(context: Context) {
        dataStore = context.datsStore
    }
    ...
    private fun readLongFlow(key: String, default: Long): Flow<Long> =
        dataStore.data.catch {
            if (it is IOException) {
                it.printStackTrace()
                emit(emptyPreferences())
    ...
    fun readStringData(nickName: String, default: String = ""): String {
        var value = ""
        runBlocking {
            dataStore.data.first {
                value = it[stringPreferencesKey(value)] ?: default
                true
            }
        }
        return value
